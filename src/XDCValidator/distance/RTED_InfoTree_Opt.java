...
package XDCValidator.distance;

import java.util.ArrayList;
import java.util.Arrays;

import XDCValidator.util.LabelDictionary;
import XDCValidator.util.LblTree;
...
public class RTED_InfoTree_Opt {
    ...
    private static final byte POST2_STRATEGY = 14; // strategy for Demaine (is there sth on the left/right of the heavy node)
    ...
    private byte[][] deltaBit; // stores the distances difference of a form delta(F,G)-delta(F°,G°) for every pair of subtrees, which is at most 1
        ...
        for (int i1 = 1; i1 <= i - ioff; i1++) {
            for (int j1 = 1; j1 <= j - joff; j1++) {
                ...
                if ((it1.info[POST2_LLD][i1 + ioff] == it1.info[POST2_LLD][i])
                    ...
                    da = forestdist[i1 - 1][j1] + costDel;
                    db = forestdist[i1][j1 - 1] + costIns;
                    dc = forestdist[it1.info[POST2_LLD][i1 + ioff] - 1 - ioff][it2.info[POST2_LLD][j1 + joff] - 1 - joff]
                            + (switched ? delta[j1 + joff][i1 + ioff] : delta[i1 + ioff][j1 + joff]) + u;
                    forestdist[i1][j1] = (da < db) ? ((da < dc) ? da : dc) : ((db < dc) ? db : dc);
                }
            }
        }
    ...
    private double spfR(InfoTree it1, InfoTree it2) {
        ...
        return it1.isSwitched() ? delta[it2.getCurrentNode()][it1.getCurrentNode()] + deltaBit[it2.getCurrentNode()][it1.getCurrentNode()]*costMatch
                : delta[it1.getCurrentNode()][it2.getCurrentNode()] + deltaBit[it1.getCurrentNode()][it2.getCurrentNode()]*costMatch;
    }
        ...
        for (int i1 = 1; i1 <= i - ioff; i1++) {
            for (int j1 = 1; j1 <= j - joff; j1++) {
                ...
                if ((it1.info[RPOST2_RLD][i1 + ioff] == it1.info[RPOST2_RLD][i])
                    ...
                    setDeltaBitValue(it1.info[RPOST2_POST][i1 + ioff], it2.info[RPOST2_POST][j1 + joff], (byte) ((forestdist[i1][j1] - forestdist[i1 - 1][j1 - 1] > 0) ? 1 : 0), switched);
                } else {
                    double u = 0;
                    ...
                    da = forestdist[i1 - 1][j1] + costDel;
                    db = forestdist[i1][j1 - 1] + costIns;
                    dc = forestdist[it1.info[RPOST2_RLD][i1 + ioff] - 1 - ioff][it2.info[RPOST2_RLD][j1 + joff] - 1 - joff]
                            + (switched ? delta[it2.info[RPOST2_POST][j1 + joff]][it1.info[RPOST2_POST][i1 + ioff]]
                            : delta[it1.info[RPOST2_POST][i1 + ioff]][it2.info[RPOST2_POST][j1 + joff]]) + u;
                    forestdist[i1][j1] = (da < db) ? ((da < dc) ? da : dc) : ((db < dc) ? db : dc);
                }
            }
        }
    ...
    private void computePeriod(InfoTree it1, int aVp, int aNextVp, InfoTree it2, int aStrategy) {
        ...
        if (aNextVp != -1) {
            ...
            // if strategy==LEFT use preorder to count number of left deletions from vp to vp-1
        ...
        // if aVp is a leaf => precompute table T - edit distance betwen EMPTY and all subforests of G

        // check if nextVp is the only child of vp
        if (vpSize - nextVpSize == 1) {
            // update delta from T table => dist between Fvp-1 and G was computed in previous compute period
            if (gSize == 1) {
        ...
        for (int i = gSize - 1; i >= 0; i--) {
            ...
            for (int kPrime = 1; kPrime <= k; kPrime++) {
                ...
                for (int j = jOfI; j >= 0; j--) {
                    ...
                    if (kPrime == 1) {
                        // if the direction changed from the previous period to this one use i and j of previous strategy

                        // since T is overwritten continously, thus use T_copy for getting values from previous period

                        if (aStrategy != previousStrategy) {
                            if (aStrategy == LEFT) {
                                previousI = gijForestPreorder - gPreorder; // minus preorder of G
                            } else {
                                previousI = gTreeSize - 1 - it2.info[RPOST2_POST][gTreeSize - 1 - gijForestPreorder] - gRevPreorder; // minus rev preorder of G
                            }
                    ...
                    sTable[kPrime - 1][j] = (deleteFromLeft < deleteFromRight)
                            ? ((deleteFromLeft < match) ? deleteFromLeft : match) : ((deleteFromRight < match) ? deleteFromRight : match);
            ...
            if (i > 0) {
                ...
                if (i + jOfIminus1 < gSize) {
                    ...
                    // then Fk without the root node is Fk-1 and the distance value has to be taken from previous T table.
                    if (k - 1 - 1 < 0) {
                        if (aStrategy != previousStrategy) {
                            previousI = aStrategy == LEFT ? IJ[i][jOfIminus1] - gPreorder : IJ[i][jOfIminus1] - gRevPreorder;
                            setDeltaValue(it1.info[PRE2_POST][vpPreorder], it2.info[PRE2_POST][gijOfIMinus1Preorder], tTable_copy[previousI][i + jOfIminus1 - previousI], switched);
                        } else {
                            setDeltaValue(it1.info[PRE2_POST][vpPreorder], it2.info[PRE2_POST][gijOfIMinus1Preorder], tTable_copy[i][jOfIminus1], switched);
                        }
                    } else {
                        setDeltaValue(it1.info[PRE2_POST][vpPreorder], it2.info[PRE2_POST][gijOfIMinus1Preorder], sTable[k - 1 - 1][jOfIminus1], switched);
                    }
                }
            }
